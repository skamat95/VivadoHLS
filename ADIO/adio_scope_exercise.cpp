...
int acquire_shot (void) {
  ...
  char fn[1000];
  char def[1000];
  strcpy(def, "/home/student/data_group1/try1.txt");
  getstrd(fn, def);

  FILE *fp = fopen(fn, "w");
  
  //
  // set up the AD to acquire data at 44.1 kHz and to have time stamps in 1 us resolution
  //


  adio->fgenab = 0x0;   // find the right value
  adio->adc_mode = 0x0; // find the right value
  adio->div_mode = 0x0; // find the right value
  
  //
  //  Write the code to acquire the data
  //
  ...
  //
  //  The code below is just dummy code to help you with the IO
  //  You have to decode the timestamp and voltage
  //
