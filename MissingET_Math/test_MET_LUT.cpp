...
bool makeTestData(int argc, char** argv,
...
/*	bool last = false;
	...
	for (event = 0; event < 170; event++) {
		...
		if (!makeTestData(argc, argv, rgn_in))
			return 999;
		...
		if (!writeInputFile(rgn_in, last))
			return 2;
		if (!writeOutputFile(MET, last))
			return 3;
	}

	if (!writeLinkMapHT(rgn_in, MET))
		return 4;
